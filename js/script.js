...
function encriptar(){
  ...
  let textoEncriptado = texto.replaceAll(/e|é|i|í|a|á|o|ó|u|ú|ü/mg, function (coincide){
    switch (coincide){      
      case 'e':
        return 'enter';
      case 'é':
...
function desencriptar(){
  let texto = document.querySelector(".text-area").value;
  let textoDesencriptado = texto.replace(/enter|imes|ai|ober|ufat/mg, function (coincide){
    switch (coincide){      
      case 'enter':
        return 'e';
      case 'imes':
